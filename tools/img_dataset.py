            ...
            return (
                ...
                [
                    ...
                    for p in self.filepaths[
                        idx * self.num_views : (idx + 1) * self.num_views
                    ]
                ],
            )
        else:
...
class ShapeNet55Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        all_files = sorted(glob.glob(parent_dir + "/" + set_ + "/*.jpg"))
...
class SNMVDatasetBase(torch.utils.data.Dataset):
    def __init__(self, dataset="train", num_models=0, num_views=1, shuffle=False):
        ...
        if num_models > 0:
            self.filepaths = self.filepaths[: min(num_models, len(self.filepaths))]
    ...
    def __getitem__(self, idx):
        jpg_paths, synsetId, subSynsetId = self.filepaths[idx]
        ...
        return (
            self._get_label(synsetId, subSynsetId),
...
class UnifiedDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        self.modelnet = ModelNet40Dataset(
            modelnet_version, dataset, num_modelnet_models, num_views, shuffle
        )
        self.shapenet = SNMVDatasetBase(
            dataset, num_shapenet_models, num_views, shuffle
        )
        self.sn_to_mn_dict = {
        ...
        self.shapenet.ids = [
            id for id in self.shapenet.ids if id not in self.sn_to_mn_dict.keys()
        ]
